...
class AbstractGame:
    ...
    def __init__(self, ctr) -> None:
        ...
        # Initialize Pygame
        pygame.init()
        self.graphics = pygame.display.set_mode((GAME_WIDTH, GAME_HEIGHT))
        # Set game title
        pygame.display.set_caption("Advanced AI Game (Q-Learning)")
    ...
    def UpdateFrame(self) -> None:
        action: GameActions = self.controller.GetAction(self.GetCurrentState())
    ...
    def DrawFrame(self):
        ...
        pygame.draw.rect(
            self.graphics,
            GOAL_COLOR,
            self.GetPygameRect(self.__innerState.GoalLocation)
        )
        pygame.draw.rect(
            self.graphics,
            PLAYER_COLOR,
            self.GetPygameRect(self.__innerState.PlayerEntity.entity)
        )
...
if __name__ == '__main__':
    ...
    if x and x[0] == '2':
        ...
        print(f'On Evaluation, player died {result[0]} times, while reaching the goal {result[1]} times')
